...
export class SpaceRoute{
  constructor(scene,{camera=null,curve=null,showLine=true}={}){
    ...
    this.scaleFactor=0.040; this.scaleMin=2.0; this.scaleMax=9.0; this.labelScaleK=2.0; this.labelYOffsetK=0.8;
  }
  ...
  _updateClusters(){
    const eps=this.clusterEps; const items=Object.entries(this.ships).map(([id,s])=>({id,t:s.currentT})).sort((a,b)=>a.t-b.t);
    this.cluster={};
    for(let i=0;i<items.length;){ const baseT=items[i].t; const group=[]; let j=i; for(; j<items.length && Math.abs(items[j].t-baseT)<=eps; j++) group.push(items[j]);
      const n=group.length; for(let k=0;k<n;k++){ const ord=(k%2===0)? k/2 : -(k+1)/2; this.cluster[group[k].id]={ord,n,t:group[k].t}; } i=j; }
  }
